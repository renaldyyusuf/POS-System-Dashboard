...
import {
  ChevronDown, ChevronRight, Clock, Package, Users, ListChecks, CalendarDays,
} from "lucide-react";
...
const COLUMNS: KanbanColumn[] = [
  ...
  },
  {
    ...
    dropActiveBg: "bg-slate-500/5 border-slate-400/50",
...
function TimeSlotBlock({
  ...
  return (
    <div className="rounded-2xl border border-border/60 bg-card/40 overflow-hidden shadow-lg shadow-black/15">
      ...
      {expanded && (
        <div className="p-4">
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-3 min-h-[180px]">
            {COLUMNS.map((col) => (
              <KanbanColumn
                key={col.id}
                col={col}
...
export default function ProductionSchedule() {
  ...
  const orders = useLiveQuery(() => db.orders.filter((o) => !o.is_void).toArray()) ?? [];
